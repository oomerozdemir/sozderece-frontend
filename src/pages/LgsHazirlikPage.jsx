...
const LGS_DATE = new Date("2026-06-14");

function daysLeft() {
  return Math.max(0, Math.ceil((LGS_DATE - new Date()) / (1000 * 60 * 60 * 24)));
}
...
const HOW_STEPS = [
  { num: "01", title: "Çocuğunuzu tanıyoruz", desc: "Hangi konular zayıf, nerede takılıyor, asıl sorun ne. Bunu anlamadan plan kurmak işe yaramıyor." },
  { num: "02", title: "Günlük plan, günlük takip", desc: "Her gün ne çalışacağı belli. Telefon kullanımı da planlanıyor. Ödevler takip ediliyor." },
  { num: "03", title: "Sizi de bilgilendiriyoruz", desc: "Haftalık ilerleme raporu veliye. Çocuğunuzun sürecini uzaktan görebiliyorsunuz." },
];

const COMPARISON = [
  { label: "Günlük plan", soz: true, alone: false },
  { label: "Deneme analizi", soz: true, alone: false },
  { label: "Telefon yönetimi", soz: true, alone: false },
  { label: "Veli bilgilendirmesi", soz: true, alone: false },
  { label: "Motivasyon takibi", soz: true, alone: false },
];

const TESTIMONIALS = [
  {
    quote: "Kızım çalışıyordu ama sonuç alamıyordu. İki hafta sonra ağladım — ilerliyordu.",
    author: "8. Sınıf Öğrenci Velisi",
    isParent: true,
  },
  {
    quote: "Başka koçluktan geliyorum. Orada haftalık mesaj atıyorlardı. Burada koçum her gün ne çalıştığımı biliyor.",
    author: "Elif, LGS Öğrencisi",
    isParent: false,
  },
];
...
export default function LgsHazirlikPage() {
  ...
  const days = daysLeft();
  const dailyCost = days > 0 ? Math.round(2500 / days) : 2500;
  ...
  // IntersectionObserver: hide sticky when offer section is visible
  useEffect(() => {
    ...
    <div className="min-h-screen flex flex-col bg-white font-poppins">
        ...
        <div className="max-w-3xl mx-auto text-center relative">
          <motion.div {...fadeUp}>
            ...
            <p className="text-white/70 text-lg max-[640px]:text-base mb-8">
              Her gün yanında biri var —{" "}
              <span className="text-white font-semibold">koçu, planı, sistemi.</span>
            </p>

            {/* Chips */}
            <div className="flex flex-wrap justify-center gap-2 mb-8">
              {[
                "📅 LGS'ye kadar günlük takip",
      ...
      <section className="py-20 px-5 bg-[#f8fafc]">
        <div className="max-w-5xl mx-auto">
          <motion.div {...fadeUp} className="text-center mb-12">
            <h2 className="text-3xl max-[640px]:text-2xl font-black text-[#0f172a]">Tanıdık geliyor mu?</h2>
            <p className="text-[#64748b] mt-2">Pek çok velinin yaşadığı sorunlar — çözümsüz değil.</p>
          </motion.div>
      ...
      <section className="py-20 px-5 bg-white">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-6 mb-16 max-[768px]:grid-cols-1">
            {HOW_STEPS.map((s, i) => (
              <motion.div key={i} {...fadeUp} transition={{ duration: 0.5, delay: i * 0.1 }}
                ...
                <div className="text-4xl font-black text-[#100481]/10 mb-3">{s.num}</div>
          ...
          <motion.div {...fadeUp}>
            <h3 className="text-xl font-black text-[#0f172a] text-center mb-6">Neden Sözderece?</h3>
      ...
      <section className="py-20 px-5 bg-gradient-to-br from-[#f8fafc] to-[#eff6ff]">
        <div className="max-w-5xl mx-auto">
          <motion.div {...fadeUp} className="text-center mb-12">
            <h2 className="text-3xl max-[640px]:text-2xl font-black text-[#0f172a]">
              Sadece söz değil —{" "}
              <span className="text-[#f39c12]">aileler konuşuyor</span>
            </h2>
          </motion.div>
          ...
          <div className="grid grid-cols-3 gap-5 mb-12 max-[640px]:grid-cols-1">
            {[
              ...
              { val: remaining !== null && remaining > 0 ? `${remaining}` : "Sınırlı", label: "yer kaldı" },
            ].map((s, i) => (
        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid grid-cols-2 gap-8 max-[768px]:grid-cols-1">
            ...
            <motion.div {...fadeUp}
              ...
              <h2 className="text-2xl font-black mb-2">LGS'ye Kadar Yanındayız</h2>
              <p className="text-white/60 text-sm mb-6">Tek seferlik ödeme, LGS'ye kadar tam destek.</p>
              ...
              <div className="mb-6">
                <div className="text-5xl font-black text-[#f39c12]">₺2.500</div>
                <p className="text-white/50 text-xs mt-1">LGS'ye kadar — tek seferlik</p>
                {days > 0 && (
              ...
              <ul className="space-y-2 mb-8 flex-grow">
                {["Günlük kişisel çalışma planı", "Haftalık deneme analizi", "Telefon yönetimi desteği", "Haftalık veli raporu", "Motivasyon koçluğu"].map((item, i) => (
            ...
            <motion.div {...fadeUp} transition={{ delay: 0.1 }}>
              <div id="lgs-form" ref={formRef} className="bg-[#f8fafc] rounded-3xl p-8 border border-[#e2e8f0]">
                <h3 className="text-xl font-black text-[#0f172a] mb-1">Hâlâ sorunuz var mı?</h3>
                <p className="text-[#64748b] text-sm mb-6">Formu doldurun, sizi arayalım.</p>

                {submitted ? (
                  <div className="py-10 text-center">
                    <div className="text-5xl mb-3">✅</div>
                    <h4 className="text-lg font-black text-[#0f172a] mb-1">Başvurunuz alındı!</h4>
                    <p className="text-[#64748b] text-sm">En kısa sürede sizi arayacağız.</p>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit} className="space-y-3">
                    ...
                    >
                      {submitting ? "Gönderiliyor..." : "Gönder, Sizi Arayalım →"}
                    </button>
                  </form>
                )}
              </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 p-4 bg-white/95 backdrop-blur border-t border-[#e2e8f0] shadow-[0_-4px_24px_rgba(0,0,0,0.1)]">
          <div className="max-w-lg mx-auto flex items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-sm font-bold text-[#0f172a]">
              ...
              <span className="text-[#64748b]">₺2.500 / LGS'ye kadar</span>
            </div>
            <button
              onClick={scrollToOffer}
