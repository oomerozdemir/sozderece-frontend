...
function HeroEditor({ content, setContent }) {
  ...
  return (
    <div className="space-y-4">
      <Field label="YKS Tarihi (YYYY-MM-DD)">
        <input type="date" value={content.yksDate || "2027-06-20"} onChange={(e) => set(setContent, "yksDate", e.target.value)} className={inp} />
      </Field>
      <Field label="Başlık Aksanı (renkli kısım)">
        <input value={h.titleAccent || ""} onChange={(e) => s("titleAccent", e.target.value)} className={inp} placeholder="YKS Yolculuğunda Koçun Yanında Olsun." />
      </Field>
      <Field label="Alt Başlık">
        <input value={h.subtitle || ""} onChange={(e) => s("subtitle", e.target.value)} className={inp} placeholder="Günlük plan, anlık takip, deneme analizi..." />
      </Field>
    ...
    <div className="space-y-4">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <AddBtn onClick={() => arrAdd(setContent, "painPoints.items", { title: "", desc: "" })} label="Kart Ekle" />
        </div>
        <p className="text-xs text-[#94a3b8] mb-2">İkonlar sayfada otomatik sırayla atanır, buradan düzenlenmez.</p>
        <div className="space-y-3">
          {items.map((item, i) => (
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <AddBtn onClick={() => arrAdd(setContent, "socialProof.testimonials", { quote: "", author: "", isParent: false })} label="Yorum Ekle" />
        </div>
        <div className="space-y-3">
          ...
          {plans.map((plan, i) => (
            <div key={i} className={`rounded-xl p-4 border space-y-3 ${plan.isFeatured ? "bg-[#eff6ff] border-[#3b82f6]" : "bg-[#f8fafc] border-[#e2e8f0]"}`}>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Field label="Fiyat Metni (/ 2 ay, / YKS'ye kadar...)">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Fiyat Etiketi">
            <input value={o.priceLabel || ""} onChange={(e) => set(setContent, "offer.priceLabel", e.target.value)} className={inp} placeholder="4 haftalık program" />
          </Field>
          <Field label="Ana CTA">
...
// ── Main component ────────────────────────────────────────────
export default function AdminYksPage() {
  ...
  const load = () => {
    setLoading(true);
    axios.get("/api/admin/yks-applications", { headers: authHeaders() })
      .then((r) => {
        setData(r.data);
  ...
  const handleExport = () => {
    const token = localStorage.getItem("token");
    window.open(`/api/admin/yks-applications/export?token=${token}`, "_blank");
  };
  ...
  const handleStatusChange = async (id, status) => {
    setUpdatingId(id);
    try {
      await axios.put(`/api/admin/yks-applications/${id}/status`, { status }, { headers: authHeaders() });
      setData((prev) => ({
        ...prev,
  ...
  const handleDeleteApplication = async (id) => {
    ...
    try {
      await axios.delete(`/api/admin/yks-applications/${id}`, { headers: authHeaders() });
      setData((prev) => ({
        ...prev,
  ...
  const handleSaveQuota = async () => {
    setSaving(true);
    try {
      await axios.put("/api/admin/settings/yks", { maxQuota }, { headers: authHeaders() });
      setSaved(true);
  ...
  const handleSaveContent = async () => {
    ...
    try {
      await axios.put("/api/admin/yks-content", content, { headers: authHeaders() });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-black text-[#0f172a]">🎓 YKS Yolculuğu</h2>
          <p className="text-xs text-[#64748b] mt-0.5">/yks-yolculugu sayfası yönetimi</p>
        </div>
        {data && (
                  ...
                  {filteredApplications.map((app, i) => (
                    <tr key={app.id} className={`border-t border-[#f1f5f9] ${i % 2 === 0 ? "bg-white" : "bg-[#f8fafc]"} hover:bg-[#eff6ff] transition-colors`}>
                      ...
                      <td className="px-4 py-3">
                        {app.grade ? <span className="bg-[#eff6ff] text-[#1d4ed8] text-xs font-bold px-2 py-0.5 rounded-full">{app.grade}</span> : <span className="text-[#94a3b8] text-xs">—</span>}
                      </td>
